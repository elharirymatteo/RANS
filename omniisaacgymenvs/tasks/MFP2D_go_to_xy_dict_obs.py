...
class MFP2DGoToXYDictTask(RLTask):
    def __init__(
        self,
        ...
        self.transforms = torch.zeros((self._num_actions, 16), device=self._device, dtype=torch.float32)
    ...
    def cleanup(self) -> None:
        ...
        self.obs_buf = {'state':torch.zeros((self._num_envs, self.num_observations), device=self._device, dtype=torch.float),
                        'transforms':torch.zeros((self._num_envs, self._max_actions, 16), device=self._device, dtype=torch.float)}
    ...
    def get_floating_platform(self):
        ...
        self.transforms[...] = torch.from_numpy(fp._transforms).reshape([-1,4*4])
