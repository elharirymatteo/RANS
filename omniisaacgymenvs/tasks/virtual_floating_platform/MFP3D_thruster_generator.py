from omniisaacgymenvs.tasks.virtual_floating_platform.MFP3D_core import parse_data_dict, euler_angles_to_matrix
...
class VirtualPlatform:
    ...
    def __init__(self, num_envs: int, platform_cfg: dict, device: str) -> None:
        ...
        self.transforms3D = torch.zeros((num_envs, self._max_thrusters, 4, 4), device=self._device, dtype=torch.float32)
        self.current_transforms = torch.zeros((num_envs, self._max_thrusters, 10), device=self._device, dtype=torch.float32)
    ...
    def create_unit_vector(self) -> None:
        ...
        tmp_y = torch.zeros((self._num_envs, self._max_thrusters, 2), device=self._device, dtype=torch.float32)
    ...
    def project_forces(self, forces: torch.Tensor) -> list:
        ...
        positions = T
        # Project forces
        force_vector = self.unit_vector * rand_forces.view(self._num_envs, self._max_thrusters, 1)
    ...
    def generate_base_platforms(self, num_envs: int, env_ids: torch.Tensor) -> None:
        ...
        thrust_90_x = torch.zeros((self._num_envs, self._max_thrusters), device=self._device)
        thrust_90_y = (torch.concat([torch.ones(2, device=self._device)/2.0, torch.arange(2, device=self._device)]).repeat(self._max_thrusters//4).expand(self._num_envs, self._max_thrusters) * 2 - 1) * math.pi / 2 
        thrust_90_z = (torch.concat([torch.arange(2, device=self._device), torch.ones(2, device=self._device)/2.0]).repeat(self._max_thrusters//4).expand(self._num_envs, self._max_thrusters) * 2 - 1) * math.pi / 2 
        # Generates N, four by four thruster
        thrust_offset = torch.arange(self.thruster_cfg.num_anchors, device=self._device).repeat_interleave(4).expand(self._num_envs, self._max_thrusters)/self.thruster_cfg.num_anchors * math.pi * 2
        ...
        if self.rand_cfg.kill_thrusters:
            ...
            kills = torch.multinomial(weights, num_samples=self.rand_cfg.max_thruster_kill, replacement=True)
            # Selects L indices to set to N+1
            weights = torch.ones(self._max_thrusters, device=self._device).expand(self._num_envs, -1)
            kill_ids = torch.multinomial(weights, num_samples=self.rand_cfg.max_thruster_kill, replacement=False)
            ...
            kill_mask = torch.sum(torch.nn.functional.one_hot(final_kill_ids, self._max_thrusters+1),dim=1)
        ...
        euler = torch.concatenate([thrust_90_x.view(thrust_90_x.shape + (1,)),
                                   thrust_90_y.view(thrust_90_x.shape + (1,)),
                                   thrust_90_z.view(thrust_90_x.shape + (1,))], axis=-1)
        ...
        transforms3D = transforms3D * mask.view(mask.shape + (1,1,))

        # Actions masks to define which thrusters can be used.
        action_masks[:, :] = 1 - mask.long()
        # Transforms to feed to the transformer.
        current_transforms[:, :,:6] = transforms3D[:,:,:2,:3].reshape(self._num_envs,self._max_thrusters, 6)
        ...
        if self.rand_cfg.random_permutation:
            ...
            transforms3D = torch.gather(transforms3D, 1, selected_thrusters.view(self._num_envs,self._max_thrusters, 1,1).expand(self._num_envs,self._max_thrusters,4,4))
            current_transforms = torch.gather(current_transforms, 1, selected_thrusters.view(self._num_envs,self._max_thrusters, 1).expand(self._num_envs,self._max_thrusters,10))
    ...
    def visualize(self, save_path: str=None):
        ...
        def renderColorsForQuiver3d(colors):
            colors = list(filter(lambda x: x!=(0.,0.,0.), colors))
