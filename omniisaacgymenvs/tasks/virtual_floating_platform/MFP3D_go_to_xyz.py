from omniisaacgymenvs.tasks.virtual_floating_platform.MFP3D_core import Core, parse_data_dict
from omniisaacgymenvs.tasks.virtual_floating_platform.MFP3D_task_rewards import GoToXYZReward
from omniisaacgymenvs.tasks.virtual_floating_platform.MFP3D_task_parameters import GoToXYZParameters
from omniisaacgymenvs.utils.pin3D import VisualPin3D

from omniisaacgymenvs.tasks.virtual_floating_platform.MFP2D_go_to_xy import GoToXYTask as GoToXYTask2D
...
class GoToXYZTask(GoToXYTask2D, Core):
    ...
    def __init__(self, task_param: GoToXYZParameters, reward_param: GoToXYZReward, num_envs: int, device: str) -> None:
    ...
    def get_goals(self, env_ids: torch.Tensor, targets_position: torch.Tensor, targets_orientation: torch.Tensor) -> list:
        ...
        self._target_positions[env_ids] = torch.rand((num_goals, 3), device=self._device)*self._task_parameters.goal_random_position*2 - self._task_parameters.goal_random_position
    ...
    def get_spawns(self, env_ids: torch.Tensor, initial_position: torch.Tensor, initial_orientation: torch.Tensor, step: int=0) -> list:
        ...
        initial_position[env_ids, 2] += (r)*torch.cos(phi) + self._target_positions[env_ids, 2]

        # Randomizes the orientation of the platform
        uvw = torch.rand((num_resets,3), device=self._device)
        initial_orientation[env_ids, 0] = torch.sqrt(uvw[:,0])*torch.cos(uvw[:,2]*2*math.pi)
        initial_orientation[env_ids, 1] = torch.sqrt(1 - uvw[:,0])*torch.sin(uvw[:,1]*2*math.pi)
        initial_orientation[env_ids, 2] = torch.sqrt(1 - uvw[:,0])*torch.cos(uvw[:,1]*2*math.pi)
        initial_orientation[env_ids, 3] = torch.sqrt(uvw[:,0])*torch.sin(uvw[:,2]*2*math.pi)
