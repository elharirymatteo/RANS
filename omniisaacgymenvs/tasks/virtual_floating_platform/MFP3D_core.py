...
class Core(Core2D):
    ...
    def __init__(self, num_envs: int, device: str) -> None:
        ...
        self._task_data = torch.zeros((self._num_envs, 9), device=self._device, dtype=torch.float32)
    ...
    def update_observation_tensor(self, current_state: dict) -> torch.Tensor:
        ...
        self._obs_buffer[:, 0:6] = current_state["orientation"][:,:2,:].reshape(self._num_envs, 6)
...
def parse_data_dict(dataclass: dataclass, data: dict, ask_for_validation: bool=False) -> dataclass:
    """
