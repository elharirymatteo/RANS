...
class GoToPoseTask(GoToPoseTask2D):
    ...
    def compute_reward(self, current_state: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        ...
        trace = self._heading_error[:,0,0] + self._heading_error[:,1,1] + self._heading_error[:,2,2]
    ...
    def get_goals(self, env_ids: torch.Tensor, target_positions: torch.Tensor, target_orientations: torch.Tensor) -> list:
        ...
        quat[env_ids, 1] = torch.sqrt(1 - uvw[:,0])*torch.sin(uvw[:,1]*2*math.pi)
        quat[env_ids, 2] = torch.sqrt(1 - uvw[:,0])*torch.cos(uvw[:,1]*2*math.pi)
