...
def plot_episode_data_virtual(ep_data:dict, save_dir:str, all_agents:bool = False) -> None:
    """
    ...
    if all_agents:
        ...
        if task_flag == 0: # GoToXY
            task_metrics = [plot_trajectories_GoToXY,
            ...
            task_metrics = [plot_distance_TrackXYVelocity,
                            plot_all_distances_TrackXYVelocity]
        elif task_flag == 3: # TrackXYOVelocity
            task_metrics = [plot_distance_TrackXYOVelocity,
        ...
        fig_count = plot_one_episode({k:np.array([v[0] for v in vals]) for k,vals in ep_data.items()}, save_dir+"first_ep/")

def plot_distance_GoToXY(state_history: np.ndarray, tgrid: np.ndarray, save_dir:str, best_agent:int, worst_agent:int, fig_count:int, **kwargs) -> int:
    """
...
def plot_all_distances_GoToXY(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, **kwargs) -> int:
    """
...
def plot_distance_GoToPose(state_history: np.ndarray, tgrid: np.ndarray, save_dir:str, best_agent:int, worst_agent:int, fig_count:int, **kwargs) -> int:
    """
    Plot mean, std_dev, best and worst distance over all episodes."""
    ...
    fig_count += 1
    fig, ax = plt.subplots()
    ax.plot(tgrid, all_position_distances.mean(axis=1), alpha=0.5, color='blue', label='mean_dist', linewidth = 1.5)
    ax.fill_between(tgrid, all_position_distances.mean(axis=1) - all_position_distances.std(axis=1), all_position_distances.mean(axis=1) 
                    + all_position_distances.std(axis=1), color='blue', alpha=0.4)
    ax.fill_between(tgrid, all_position_distances.mean(axis=1) - 2*all_position_distances.std(axis=1), all_position_distances.mean(axis=1) 
                    + 2*all_position_distances.std(axis=1), color='blue', alpha=0.2)
    ax.plot(tgrid, all_position_distances[:, best_agent], alpha=0.5, color='green', label='best', linewidth = 1.5)
    ax.plot(tgrid, all_position_distances[:, worst_agent], alpha=0.5, color='red', label='worst', linewidth = 1.5)
    plt.xlabel('Time steps')
    plt.ylabel('Distance [m]')
    plt.legend(['mean',  'best', 'worst', '1-std', '2-std'], loc='best')
    ...
    fig_count += 1
    fig, ax = plt.subplots()
    ax.plot(tgrid, all_heading_distances.mean(axis=1), alpha=0.5, color='blue', label='mean_dist', linewidth = 1.5)
    ax.fill_between(tgrid, all_heading_distances.mean(axis=1) - all_heading_distances.std(axis=1), all_heading_distances.mean(axis=1) 
                    + all_heading_distances.std(axis=1), color='blue', alpha=0.4)
    ax.fill_between(tgrid, all_heading_distances.mean(axis=1) - 2*all_heading_distances.std(axis=1), all_heading_distances.mean(axis=1) 
                    + 2*all_heading_distances.std(axis=1), color='blue', alpha=0.2)
    ax.plot(tgrid, all_heading_distances[:, best_agent], alpha=0.5, color='green', label='best', linewidth = 1.5)
    ax.plot(tgrid, all_heading_distances[:, worst_agent], alpha=0.5, color='red', label='worst', linewidth = 1.5)
    plt.xlabel('Time steps')
...
def plot_all_distances_GoToPose(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, **kwargs) -> int:
    """
    Plot all distances over all episodes."""
    ...
    for j in range(all_position_distances.shape[1]):
        ax.plot(tgrid, all_position_distances[:, j], alpha=1., color=cmap(j % cmap.N), linewidth = 1.0)
    plt.xlabel('Time steps')
    plt.ylabel('Distance [m]')
    ...
    for j in range(all_heading_distances.shape[1]):
        ax.plot(tgrid, all_heading_distances[:, j], alpha=1., color=cmap(j % cmap.N), linewidth = 1.0)
    plt.xlabel('Time steps')
    plt.ylabel('Distance [rad]')
...
def plot_distance_TrackXYVelocity(state_history: np.ndarray, tgrid: np.ndarray, save_dir:str, best_agent:int, worst_agent:int, fig_count:int, **kwargs) -> int:
    """
    Plot mean, std_dev, best and worst distance over all episodes."""
...
def plot_all_distances_TrackXYVelocity(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, **kwargs) -> int:
    """
    Plot all distances over all episodes."""
...
def plot_distance_TrackXYOVelocity(state_history: np.ndarray, tgrid: np.ndarray, save_dir:str, best_agent:int, worst_agent:int, fig_count:int, **kwargs) -> int:
    """
    Plot mean, std_dev, best and worst distance over all episodes."""
    ...
    fig_count += 1
    fig, ax = plt.subplots()
    ax.plot(tgrid, all_xy_distances.mean(axis=1), alpha=0.5, color='blue', label='mean_dist', linewidth = 1.5)
    ax.fill_between(tgrid, all_xy_distances.mean(axis=1) - all_xy_distances.std(axis=1), all_xy_distances.mean(axis=1) 
                    + all_xy_distances.std(axis=1), color='blue', alpha=0.4)
    ax.fill_between(tgrid, all_xy_distances.mean(axis=1) - 2*all_xy_distances.std(axis=1), all_xy_distances.mean(axis=1) 
                    + 2*all_xy_distances.std(axis=1), color='blue', alpha=0.2)
    ax.plot(tgrid, all_xy_distances[:, best_agent], alpha=0.5, color='green', label='best', linewidth = 1.5)
    ax.plot(tgrid, all_xy_distances[:, worst_agent], alpha=0.5, color='red', label='worst', linewidth = 1.5)
    plt.xlabel('Time steps')
    plt.ylabel('Distance [m/s]')
    plt.legend(['mean',  'best', 'worst', '1-std', '2-std'], loc='best')
    ...
    fig_count += 1
    fig, ax = plt.subplots()
    ax.plot(tgrid, all_omega_distances.mean(axis=1), alpha=0.5, color='blue', label='mean_dist', linewidth = 1.5)
    ax.fill_between(tgrid, all_omega_distances.mean(axis=1) - all_omega_distances.std(axis=1), all_omega_distances.mean(axis=1) 
                    + all_omega_distances.std(axis=1), color='blue', alpha=0.4)
    ax.fill_between(tgrid, all_omega_distances.mean(axis=1) - 2*all_omega_distances.std(axis=1), all_omega_distances.mean(axis=1) 
                    + 2*all_omega_distances.std(axis=1), color='blue', alpha=0.2)
    ax.plot(tgrid, all_omega_distances[:, best_agent], alpha=0.5, color='green', label='best', linewidth = 1.5)
    ax.plot(tgrid, all_omega_distances[:, worst_agent], alpha=0.5, color='red', label='worst', linewidth = 1.5)
    plt.xlabel('Time steps')
    plt.ylabel('Distance [m/s]')
    plt.legend(['mean',  'best', 'worst', '1-std', '2-std'], loc='best')
...
def plot_all_distances_TrackXYOVelocity(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, **kwargs) -> int:
    """
    Plot all distances over all episodes."""
    ...
    for j in range(all_xy_distances.shape[1]):
        ax.plot(tgrid, all_xy_distances[:, j], alpha=1., color=cmap(j % cmap.N), linewidth = 1.0)
    plt.xlabel('Time steps')
    plt.ylabel('Distance [m/s]')
    ...
    for j in range(all_omega_distances.shape[1]):
        ax.plot(tgrid, all_omega_distances[:, j], alpha=1., color=cmap(j % cmap.N), linewidth = 1.0)
    plt.xlabel('Time steps')
...
def plot_reward(reward_history:np.ndarray, state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, best_agent:int, worst_agent:int, fig_count:int, **kwargs) -> int:
    """
...
def plot_velocities(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, best_agent:int, worst_agent:int, fig_count:int, **kwargs) -> int:
    """
...
def plot_trajectories_GoToXY(state_history:np.ndarray, save_dir:str, fig_count:int, **kwargs) -> int:
    """
...
def plot_one_episode(ep_data:dict, save_dir:str = None, show:bool = False, debug:bool = False, fig_count:int = 0) -> None:
    """
    ...
    shared_metrics = [plot_single_linear_vel,
    ...
    if task_flag == 0: # GoToXY
        ...
        task_metrics = [plot_single_TrackXYVelocity_distance_to_target,
                        plot_single_TrackXYVelocity_log_distance_to_target]
    elif task_flag == 3: # TrackXYOVelocity
        task_data_label = ['error_vx', 'error_vy', 'error_omega']
        task_metrics = [plot_single_TrackXYOVelocity_distance_to_target,
    ...
    args = {"state_history": state_history,
            "control_history": control_history,
            "reward_history": reward_history,
            "save_dir": save_dir,
            "fig_count": fig_count,
...
def plot_single_linear_vel(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_angular_vel(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_heading_cos_sin(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_absolute_heading(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_action_histogram(control_history:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_rewards(reward_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_xy_position_error(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_heading_error(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_xy_position_heading(state_history:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
    Plot position of a single agent."""
...
def plot_single_GoToXY_distance_to_target(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_GoToXY_log_distance_to_target(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
...
def plot_single_GoToPose_distance_to_target(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
    Plot distance to target of a single agent."""
...
def plot_single_GoToPose_log_distance_to_target(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
    Plot log distance to target of a single agent."""
...
def plot_single_TrackXYVelocity_distance_to_target(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
    Plot distance to target of a single agent."""
    ...
    plt.figure(fig_count)
    plt.clf()
    plt.plot(tgrid, np.linalg.norm(np.array([vel_error[:, 0], vel_error[:, 1]]), axis=0),
             color=cm.get_cmap('tab20')(0))
    plt.xlabel('Time steps')
    plt.ylabel('Distance [m/s]')
    plt.legend(['abs dist'], loc='best')
...
def plot_single_TrackXYVelocity_log_distance_to_target(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
    Plot log distance to target of a single agent."""
...
def plot_single_TrackXYOVelocity_distance_to_target(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
    Plot distance to target of a single agent."""
    ...
    plt.figure(fig_count)
    plt.clf()
    plt.plot(tgrid, np.abs(omega_error),
             color=cm.get_cmap('tab20')(0))
    plt.xlabel('Time steps')
    plt.ylabel('Distance [rad/s]')
    plt.legend(['abs dist'], loc='best')
...
def plot_single_TrackXYOVelocity_log_distance_to_target(state_history:np.ndarray, tgrid:np.ndarray, save_dir:str, fig_count:int, show:bool, **kwargs) -> int:
    """
    Plot log distance to target of a single agent."""
